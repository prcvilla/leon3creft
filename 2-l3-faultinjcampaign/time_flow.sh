...
declare -i NDETNOK=0
declare -i I=0
declare -i RUNTIMENDETOK=0
declare -i RUNTIMERECOVERED=0
...
DEBUG echo $PROGSTATUS
	...
	if [ $AHBREQ -gt 0 ]; then
		...
		if [[ ( $AHBGRT == $RECDONE ) && ( $ITERS == $NITERS) && ( $PROGFIN == 1 ) ]]; then
			RECOVERED+=1
			RUNTIMERECOVERED+=$(grep '>>>>>>>>>>>>>>>' $FILE | awk '{print $2}')
			DEBUG echo $RUNTIME
		fi
		...
		if [[ ( $AHBGRT == 0 ) && ( $RECDONE == 0 ) && ( $ITERS == $NITERS) && ( $PROGFIN == 1 ) ]]; then
			NDETOK+=1
			RUNTIMENDETOK+=$(grep '>>>>>>>>>>>>>>>' $FILE | awk '{print $2}')
			DEBUG echo $RUNTIME
		fi
...
echo -n "RUNTIMENDETOK:$RUNTIMENDETOK Mean: "
echo $(( RUNTIMENDETOK / NDETOK ))
echo -n "RUNTIMENRECOVERED:$RUNTIMERECOVERED Mean: "
echo $(( RUNTIMERECOVERED / RECOVERED ))
